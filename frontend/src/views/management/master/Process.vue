...
async function fetchLines(){const r=await api.get('/api/master/line',{params:{size:999}});lines.value=r.data.data;}
async function fetchFactories(){const r=await api.get('/api/master/factory',{params:{size:999}});factories.value=r.data.data;}
async function fetchData(){loading.value=true;try{const p:any={page:page.value,size:50};if(searchNm.value)p.search=searchNm.value;const r=await api.get('/api/master/process',{params:p});items.value=r.data.data;total.value=r.data.total;totalPages.value=r.data.totalPages;selectedIdx.value=-1;}finally{loading.value=false;}}
