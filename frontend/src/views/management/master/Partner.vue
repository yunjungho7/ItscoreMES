  ...
  <div class="page-view">
    <section class="search-section">
      ...
      <div class="search-row">
        ...
        <label>업체구분</label><select v-model="searchType"><option value="">전체</option><option value="매입">매입</option><option value="매출">매출</option><option value="외주">외주</option></select>
        <label>사용여부</label><select v-model="searchUse"><option value="">전체</option><option value="1">사용</option><option value="0">미사용</option></select>
    ...
    <FormModal :visible="showModal" :title="editMode?'거래처 수정':'거래처 등록'" width="720px" :showDelete="editMode" @close="showModal=false" @save="handleSave" @delete="handleDelete">
      <div class="fg3">
        <div class="ff"><label>업체코드</label><input v-model="form.COMPANYCD" :disabled="editMode"/></div>
        <div class="ff"><label>사업장</label><input v-model="form.PLANTCD"/></div>
        <div class="ff"><label>업체구분</label><input v-model="form.COMPANYTYPE"/></div>
        <div class="ff s2"><label>업체명</label><input v-model="form.COMPANYNM"/></div>
...
async function handleSave(){if(!form.COMPANYCD||!form.PLANTCD){alert('업체코드와 사업장을 입력하세요.');return;}try{if(editMode.value){await api.put(`/api/master/company/${form.COMPANYCD}`,form);alert('수정되었습니다.');}else{await api.post('/api/master/company',form);alert('등록되었습니다.');}showModal.value=false;fetchData();}catch(e){}}
...
.fg3{display:grid;grid-template-columns:repeat(3,1fr);gap:14px 18px}.ff{display:flex;flex-direction:column;gap:5px}.ff.s2{grid-column:span 2}.ff.s3{grid-column:span 3}.ff label{font-size:.82rem;font-weight:600;color:#64748b}.ff input,.ff select{padding:9px 12px;border:1px solid #e2e8f0;border-radius:8px;font-size:.88rem;transition:border-color .15s}.ff input:focus,.ff select:focus{border-color:#667eea;outline:none;box-shadow:0 0 0 3px rgba(102,126,234,.1)}.ff input:disabled{background:#f8fafc;color:#94a3b8}
</style>
