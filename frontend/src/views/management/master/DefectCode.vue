...
async function fetchAllCodes(){const r=await api.get('/api/master/failcode',{params:{size:999}});allCodes.value=r.data.data;}
async function fetchData(){loading.value=true;try{const p:any={page:page.value,size:50};if(searchNm.value)p.search=searchNm.value;const r=await api.get('/api/master/failcode',{params:p});items.value=r.data.data;total.value=r.data.total;totalPages.value=r.data.totalPages;selectedIdx.value=-1;}finally{loading.value=false;}}
