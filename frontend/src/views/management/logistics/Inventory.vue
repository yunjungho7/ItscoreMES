  ...
  <div class="page-view">
    <section class="search-section">
      ...
      <div class="search-row">
        ...
        <label>품목구분</label>
        <select v-model="partType"><option value="">전체</option></select>
        <label>공정</label>
...
import { ref, onMounted } from 'vue'; import api from '../../../api'; import DataGrid from '../../../components/common/DataGrid.vue';
const plantCd=ref(''),warehouseCd=ref(''),lineCd=ref(''),partType=ref(''),processCd=ref(''),searchText=ref(''),locationCd=ref(''),lotNo=ref('');
const plants=ref<any[]>([]),warehouses=ref<any[]>([]);
const mCols=[{key:'PARTNO',label:'품번',width:'130px'},{key:'PARTNM',label:'품명',width:'150px'},{key:'STANDARD',label:'규격',width:'100px'},{key:'PARTTYPE',label:'품목구분',width:'80px'},{key:'UNIT',label:'단위',width:'60px'},{key:'SAFETYSTOCK',label:'안전재고',width:'80px'},{key:'GOODQTY',label:'양품수량',width:'90px'},{key:'FAILQTY',label:'불량수량',width:'80px'},{key:'TOTALQTY',label:'현재고수량',width:'100px'}];
