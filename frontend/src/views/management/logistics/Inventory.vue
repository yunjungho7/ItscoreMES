...
async function fetchWarehouses(){try{const r=await api.get('/api/master/warehouse',{params:{size:100}});warehouses.value = Array.isArray(r.data?.data) ? r.data.data : (Array.isArray(r.data?.data?.data) ? r.data.data.data : (r.data?.data || []));}catch{}}
async function fetchCodes(){try{const r=await api.get('/api/master/code',{params:{size:9999}});allCodes.value = Array.isArray(r.data?.data) ? r.data.data : (Array.isArray(r.data?.data?.data) ? r.data.data.data : (r.data?.data || []));}catch{}}
async function fetchData(){ld.value=true;si.value=-1;sel.value=null;dRows.value=[];try{const p:any={page:pg.value,size:50};if(plantCd.value)p.plant_cd=plantCd.value;if(warehouseCd.value)p.warehouse_cd=warehouseCd.value;if(lineCd.value)p.line_cd=lineCd.value;if(processCd.value)p.process_cd=processCd.value;if(partType.value)p.part_type=partType.value;if(lotNo.value)p.lot_no=lotNo.value;if(locationCd.value)p.location_cd=locationCd.value;if(searchText.value)p.search=searchText.value;const r=await api.get('/api/inventory/list',{params:p});const raw = Array.isArray(r.data?.data) ? r.data.data : (Array.isArray(r.data?.data?.data) ? r.data.data.data : (r.data?.data || []));mRows.value=raw.map((item:any)=>({...item,PARTTYPE_NM:getPartTypeName(item.PARTTYPE)}));tot.value=(r.data?.data?.total ?? r.data?.total ?? 0);tp.value=(r.data?.data?.totalPages ?? r.data?.totalPages ?? 0);}finally{ld.value=false;}}
async function onMaster(row:any,idx:number){si.value=idx;sel.value=row;dl.value=true;try{const r=await api.get(`/api/inventory/detail/${row.PARTNO}`);dRows.value = Array.isArray(r.data) ? r.data : (r.data?.data || []);}finally{dl.value=false;}}
