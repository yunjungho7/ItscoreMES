  ...
  <div class="page-view">
    ...
    <div class="split-grids">
      ...
      <div class="detail-wrap">
        <div class="dh">
          <div class="dh-title">작업실적 <span v-if="sel">- {{ sel.WORKORDNO }}</span></div>
          <div class="dh-actions">
            <button class="btn-cancel" @click="cancelResult" :disabled="!selResult">❌ 작업실적취소</button>
          </div>
        </div>
...
const dCols = [
  ...
  { key: 'LOTNO', label: '작업 LOT 번호', width: '140px' },
  { key: 'PARTNO', label: '품번', width: '120px' },
  { key: 'PARTNM', label: '품명', width: '130px' },
  { key: 'STANDARD', label: '규격', width: '80px' },
  { key: 'UNIT', label: '단위', width: '50px' },
  { key: 'ORDQTY', label: '지시수량', width: '70px', align: 'right' },
  { key: 'PRODQTY', label: '생산실적수량', width: '80px', align: 'right' },
  { key: 'GOODQTY', label: '양품수량', width: '70px', align: 'right' },
  { key: 'DEFECTQTY', label: '불량수량', width: '70px', align: 'right' },
  { key: 'LOTSTATE', label: '상태', width: '70px' },
  { key: 'WORKERCNT', label: '작업자 인원수', width: '80px', align: 'right' },
  { key: 'STARTDATE', label: '시작일자', width: '100px' },
];
...
async function fetchDetail(workordno: string) {
  dl.value = true;
  try {
    const r = await api.get(`/api/production/status/result/${workordno}`);
    dRows.value = r.data || [];
...
async function cancelResult() {
  if (!selResult.value) {
    alert('취소할 작업실적을 선택하세요.');
    return;
  }
  if (!confirm(`선택한 작업실적(LOT: ${selResult.value.LOTNO})을 취소하시겠습니까?`)) return;
  
  try {
    await api.delete(`/api/production/status/result/${selResult.value.LOTNO}`);
    alert('작업실적이 취소되었습니다.');
    // 디테일 목록 재조회
    if (sel.value) {
      di.value = -1; selResult.value = null;
      await fetchDetail(sel.value.WORKORDNO);
    }
  } catch (err: any) {
...
.dh { display: flex; justify-content: space-between; align-items: center; padding: 8px 16px; background: #eaf2f8; border-bottom: 1px solid #d6eaf8; }
.dh-title { font-size: .85rem; font-weight: 600; color: #1a5276; }
.dh-title span { color: #2980b9; }
.dh-actions { display: flex; gap: 8px; }
.btn-cancel { background: #fff; color: #c0392b; border: 1px solid #eab8b4; padding: 5px 12px; border-radius: 4px; font-weight: 600; font-size: .8rem; cursor: pointer; transition: all 0.2s; }
.btn-cancel:hover:not(:disabled) { background: #fdf2f1; }
.btn-cancel:disabled { color: #ccc; border-color: #eee; cursor: not-allowed; background: #fafafa; }
</style>
