...
@router.get("/api/status/production", summary="생산현황 목록 조회")
...
@router.get("/api/status/production/{lot_no}/defects", summary="생산실적 불량 상세 조회")
def get_production_defects(lot_no: str):
    return production_status_service.get_defects(lot_no)

@router.get("/api/status/defect", summary="불량현황 목록 조회")
