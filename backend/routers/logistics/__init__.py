...
@router.get("/order/list", summary="수주 목록 조회")
def get_orders(start_date: Optional[str] = None, end_date: Optional[str] = None,
    ...
    result = order_service.get_all(start_date=start_date, end_date=end_date,
                                 plant_cd=plant_cd, company_cd=company_cd,
                                 search=search, page=page, size=size)
    return {"data": result}

@router.get("/order/detail/{order_no}", summary="수주 상세 조회")
...
@router.get("/order/detail/{order_no}/items", summary="수주 품목 목록")
def get_order_details(order_no: int):
    result = order_service.get_details(order_no)
    return {"data": result}

@router.post("/order/create", summary="수주 등록", status_code=201)
def create_order(body: OrderCreate):
    return order_service.create(body.model_dump())

@router.put("/order/update/{order_no}", summary="수주 수정")
def update_order(order_no: int, body: OrderUpdate):
    return order_service.update(order_no, body.model_dump())

@router.delete("/order/delete/{order_no}", summary="수주 삭제")
...
@router.get("/purchase/plan", summary="생산계획(발주대상) 목록 조회")
def get_plan_list(plant_cd: Optional[str] = None, start_date: Optional[str] = None,
    ...
    result = purchase_service.get_plan_list(plant_cd=plant_cd, start_date=start_date,
                                           end_date=end_date, search=search)
    return {"data": result}

@router.post("/purchase/plan", summary="생산계획 등록", status_code=201)
def create_plan(body: ProducePlanCreate):
    purchase_service.create_plan(body.model_dump())
    return {"statusCode": 201, "message": "등록되었습니다."}

@router.get("/purchase/bom/{par_partno}", summary="BOM 자재 전개")
def get_bom_children(par_partno: str):
    result = purchase_service.get_bom_children(par_partno)
    return {"data": result}

@router.post("/purchase/order", summary="발주 등록 (BOM 기반)", status_code=201)
def create_purchase_order(body: PurchaseOrderCreate):
    return purchase_service.create_purchase_order(body.model_dump())

@router.get("/purchase/list", summary="발주 현황(목록) 조회")
def get_purchase_orders(start_date: Optional[str] = None, end_date: Optional[str] = None,
    ...
    result = purchase_service.get_purchase_orders(start_date=start_date, end_date=end_date,
                                                 plant_cd=plant_cd, company_cd=company_cd,
                                                 search=search, include_completed=include_completed)
    return {"data": result}

@router.get("/purchase/detail/{order_num}", summary="발주 현황 상세 품목 조회")
def get_purchase_order_details(order_num: str):
    result = purchase_service.get_purchase_order_details(order_num)
    return {"data": result}
...
@router.get("/receive/list", summary="입고 목록 조회")
def get_receives(start_date: Optional[str] = None, end_date: Optional[str] = None,
    ...
    result = receive_service.get_all(start_date=start_date, end_date=end_date,
                                    plant_cd=plant_cd, company_cd=company_cd,
                                    order_num=order_num, search=search,
                                    page=page, size=size)
    return {"data": result}

@router.get("/receive/summary", summary="입고 현황 요약 통계")
def get_receive_summary():
    result = receive_service.get_summary()
    return {"data": result}

@router.get("/receive/detail/{warehouse_num}/items", summary="입고 상세 품목")
def get_receive_details(warehouse_num: str):
    return receive_service.get_details(warehouse_num)

@router.post("/receive/order", summary="입고 등록 (발주기반)", status_code=201)
def create_receive_from_order(data: ReceiveCreate):
    return receive_service.create(data.model_dump())

@router.post("/receive/create", summary="입고 등록", status_code=201)
def create_receive(body: ReceiveCreate):
    return receive_service.create(body.model_dump())

@router.delete("/receive/delete/{warehouse_num}", summary="입고 삭제(취소)")
...
@router.get("/receive/po-list", summary="발주 목록 조회 (입고용)")
...
@router.get("/receive/po-detail/{order_num}", summary="발주 상세 조회 (입고용)")
...
@router.get("/inventory/list", summary="재고 현황 조회")
def get_inventory(plant_cd: Optional[str] = None, warehouse_cd: Optional[str] = None,
                  ...
                  page: int = 1, size: int = 50):
    result = inventory_service.get_all(plant_cd=plant_cd, warehouse_cd=warehouse_cd,
...
@router.get("/inventory/summary", summary="재고 현황 요약 통계")
def get_inventory_summary():
    result = inventory_service.get_summary()
    return {"data": result}

@router.get("/inventory/detail/{partno}", summary="품목별 LOT 재고 상세")
def get_inventory_detail(partno: str):
    return inventory_service.get_detail(partno)

@router.put("/inventory/update-qty", summary="재고수량 수정")
...
@router.get("/shipment/list", summary="출하지시 목록 조회")
...
def get_shipment_summary():
    result = shipment_service.get_summary()
    return {"data": result}
...
@router.delete("/shipment/delete/{shipment_no}", summary="출하지시 삭제")
