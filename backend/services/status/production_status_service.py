...
class ProductionStatusService:
    def __init__(self):
        self.mapper = XMLMapper(os.path.join(BASE_DIR, 'sql/status/production.xml'))

    def get_all(self, start_date=None, end_date=None, plant_cd=None,
                lot_no=None, part_no=None, search=None, page=1, size=50):
        params = {}
        ...
        # Count
        q = self.mapper.get_query('countProductionStatus', params)
        ...
        params['limit'] = size
        q = self.mapper.get_query('selectProductionStatus', params)
        values = tuple(params.get(name) for name in q['params'])
        cursor.execute(q['query'], values)
    ...
    def get_defects(self, lot_no):
        params = {'LOTNO': lot_no}
        q = self.mapper.get_query('selectProductionDefects', params)
        values = tuple(params.get(name) for name in q['params'])
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(q['query'], values)
