#!/bin/bash
# PFMES Production Deployment Script (Ultimate)
# 단일 소스로 운영 환경을 완벽히 분리 운영합니다.
...
# .env.production 파일 존재 확인
if [ ! -f ".env.production" ]; then
    echo -e "${RED}Error: frontend/.env.production 파일이 없습니다.${NC}"
    exit 1
fi
...
echo -e "${GREEN}   - 접속 URL: http://$(hostname -I | awk '{print $1}')${NC}"
